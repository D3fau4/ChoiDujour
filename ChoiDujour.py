...
def print_welcome():
	...
	print('uses linkle by MegatonHammer (https://github.com/MegatonHammer/linkle)')
	print('uses hactoolnet by Thealexbarney (https://github.com/Thealexbarney/LibHac)')
...
try:
	...
	for fileHash in jayson['files']:
		...
		counter = 1
		while counter < 10:
			hactoolnet_process = subprocess.Popen([
											hactoolnet_exe,
											'-t',
											'save',
											'-k',
			...
			std_out, std_err = hactoolnet_process.communicate()
			
			#print std_out

			counter = counter + 1
			
			if hactoolnet_process.returncode != 0:
				err_msg = "%s. Code: %s" % (std_err.strip().decode(), hactoolnet_process.returncode)
				raise Exception(err_msg)
			else:
			...
			print("ERROR: Re-signing save")
						
		set_file_attributes(filePath, fileInfo.attrs)
		while counter == 10:
			print("")
			print("ERROR singing the save")
			print("")
			sys.exit(0)
